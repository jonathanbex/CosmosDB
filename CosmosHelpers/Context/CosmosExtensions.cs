...
namespace CosmosHelpers.Context
{
    ...
    public static async Task<Container> EnsureCollectionAsync(this Microsoft.Azure.Cosmos.Database database, object collection = null, string? optionalKey = null, string? optionalName = null, Type? type = null)
 {
   ...
   {
     bool HasPartitionKey(Type t) =>
   t.GetProperties().Any(p => p.GetCustomAttribute<JsonPropertyAttribute>()?.PropertyName == "partitionKey");

     if (!HasPartitionKey(type ?? collection?.GetType()))
     {
       throw new Exception("The model is missing a property with [JsonProperty(PropertyName = \"partitionKey\")].");
     }
   }
